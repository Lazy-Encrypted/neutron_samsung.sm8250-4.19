...
#define INPUT_BOOST_DURATION_MS 500
#define LPM_BIAS_HYST_BOOST_VALUE 500
enum {
	SCREEN_OFF,
	MAX_BOOST
};
struct boost_drv {
	...
	unsigned long state;
    struct pm_qos_request lpm_bias_qos;
    bool lpm_boosted;
};
...
static void update_lpm_boost(struct boost_drv *b, unsigned long state)
{
	bool should_boost = !test_bit(SCREEN_OFF, &state) &&
			     test_bit(MAX_BOOST, &state);

	if (should_boost == b->lpm_boosted)
		return;

	pm_qos_update_request(&b->lpm_bias_qos,
			      should_boost ? LPM_BIAS_HYST_BOOST_VALUE :
					     PM_QOS_BIAS_HYST_DEFAULT_VALUE);
	b->lpm_boosted = should_boost;
}
...
/* --- Input boost handler --- */
static void cpu_event_boost_input_event(struct input_handle *handle,
					unsigned int type, unsigned int code,
					int value)
{
	struct boost_drv *b = handle->handler->private;
	__cpu_event_boost_kick_max(b, INPUT_BOOST_DURATION_MS);
	devfreq_boost_kick_duration(DEVFREQ_CPU_LLCC_DDR_BW, INPUT_BOOST_DURATION_MS);
}
static int cpu_event_boost_input_connect(struct input_handler *handler,
					 ...
					 const struct input_device_id *id)
{
	...
	handle->name = "cpu_event_boost_handle";
...
static void cpu_event_boost_input_disconnect(struct input_handle *handle)
{
	input_close_device(handle);
	input_unregister_handle(handle);
	kfree(handle);
}
static const struct input_device_id cpu_event_boost_ids[] = {
...
static struct input_handler cpu_event_boost_input_handler = {
	.event		= cpu_event_boost_input_event,
	.connect	= cpu_event_boost_input_connect,
	.disconnect	= cpu_event_boost_input_disconnect,
	.name		= "cpu_event_boost_handler",
	.id_table	= cpu_event_boost_ids,
};
...
pm_qos_add_request(&b->lpm_bias_qos, PM_QOS_BIAS_HYST,
			   PM_QOS_BIAS_HYST_DEFAULT_VALUE);
	...
	cpu_event_boost_input_handler.private = b;
	ret = input_register_handler(&cpu_event_boost_input_handler);
	if (ret) {
	...
	if (IS_ERR(thread)) {
		...
		goto unregister_input_handler;
	}
	return 0;
unregister_input_handler:
	input_unregister_handler(&cpu_event_boost_input_handler);
unregister_fb_notif:
