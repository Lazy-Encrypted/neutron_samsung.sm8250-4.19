 ...
 * @capacity: Capacity of the target CPU (kernel_scale_capacity units).
 *
 * This function is only to be called from the scheduler tick to adjust the
 * performance level of a CPU as indicated by the scheduler, which calls it
 * every time it computes a new value of @target_perf and every TICK_NSEC
 * nanoseconds, so it is expected to be optimized for that use case.
 *
